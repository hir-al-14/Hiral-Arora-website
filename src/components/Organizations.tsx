const organizations = [
  ...
  { name: "Void Club", description: "", role: "Engineering Team", detail: "High school", logo: "void.jpg", url: "https://www.instagram.com/thevoidclub/", present: false, style: "void", linkLabel: "View Instagram" },
];
        ...
        <div className="organizations-grid">
          {organizations.map(org => (
            <a className="organization" href={org.url} target="_blank" rel="noreferrer" key={org.name}>
              <div className={`organization-logo ${org.style}`}>{org.logo ? <img src={`/images/organizations/${org.logo}`} alt={`${org.name} logo`} loading="lazy" /> : <span aria-hidden="true">V</span>}</div>
              ...
              {org.role && <p className="organization-role">{org.role}</p>}
              {org.description && <p className="organization-description">{org.description}</p>}
              <span className="organization-link">{org.linkLabel ?? "Visit website"} <span aria-hidden="true">↗</span></span>
            </a>
          ))}
        </div>
      </div>
